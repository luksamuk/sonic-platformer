...
impl TileViewer {
    ...
    pub fn reload(&mut self, context: &mut Context) -> GameResult {
        self.tiles = Some(SpriteAtlas::new(context, &self.tile_path, glam::vec2(8.0, 8.0))?);
        Ok(())
    }
    ...
    pub fn update(&mut self, context: &mut Context) -> GameResult {
        if self.tiles.is_some() {
            ...
            atlas.clear();
            for j in 0..max_columns { // columns
                for i in 0..tiles_per_column { // lines
                    let current_frame = i + (j * tiles_per_column);
                    let hotspot = glam::vec2(
                        5.0 + (8.0 * scale_factor) + (j as f32 * 100.0) + (j as f32 * (8.0 * scale_factor)),
                        70.0 + (8.0 * scale_factor) + (i as f32 * (8.0 * scale_factor)) + (i as f32 * 10.0),
                    );
                    let text_position = glam::vec2(
                        hotspot.x + (8.0 * scale_factor),
                        hotspot.y - 8.0,
                    );
